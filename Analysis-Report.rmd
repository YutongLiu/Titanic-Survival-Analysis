...
We want to predict the survival of Titanic passengers. In the dataset, **survival** is the outcome we will predict in testset.  
...
#### Text Editing  
After checking the variable names, fortunately there is no space among the variable names and they are almost easy to use. Now I just standardize them in lower case.
...
I think variables **survived** and **sex** are actually nominal variable, although they are saved as integer.
So here I transfer them to factor.
```{r transfer_variable_type}
...
total_clean[where(is.na(total_clean$embarked)==TRUE),]
# Fare vs Embarked
gfare <- ggplot(total_clean,aes(x=embarked,y=fare))
gfare+geom_boxplot()
...
cabin_survived$cabib<-gsub("[0-9]","",cabin_survived$cabib)

# Coerce into a factor vector
unique(cabin_survived$cabib)
cabin_survived$cabib<-as.factor(cabin_survived$cabib)
...
levels(cabin_survived$cabib)<-cabin_level
```
...
### Conclusions
In this project, I tried lots of ways to process dataset and I got a max categorization accuracy about 0.79.
What surprises me is that the most important factors that impact survival status is passenger title, gender and ticket fare, passenger age is not included.
